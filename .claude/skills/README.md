...
| `storytelling/` | 味気ない事実・データを引き込まれるナラティブに変換する（内容は正確なまま） |
| `headline-generator/` | 見出し案を型別に量産し予測クリック率で評価・TOP3ランク付け |
| `case-study/` | クライアントの事実を説得力ある導入事例にまとめる（現在形・数字3つ以上・プルクォート） |
